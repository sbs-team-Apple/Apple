...
public class UserController {
    ...
    @PreAuthorize("isAuthenticated()")
    @GetMapping("/exchange")
    public String exchangePage(Model model, Principal principal) {
        String username = principal.getName();
        SiteUser user = userService.getUserbyName(username);
        int receivedCyberMoney = user.getReceivedCyberMoney();
        model.addAttribute("receivedCyberMoney", receivedCyberMoney);
        model.addAttribute("user", user);
    ...
    public String applyExchange(@RequestParam("exchangeAmount") int exchangeAmount, Principal principal, Model model) {
        String username = principal.getName();
        SiteUser user = userService.getUserbyName(username);
        int receivedCyberMoney = user.getReceivedCyberMoney();

        if (exchangeAmount < 1000) {
            model.addAttribute("error_message", "1000 이상 입력 해주세요.");
            model.addAttribute("receivedCyberMoney", receivedCyberMoney);
            return "pay/exchange";
        }

        if (exchangeAmount > receivedCyberMoney) {
            model.addAttribute("error_message", "사이버 머니가 부족합니다.");
            model.addAttribute("receivedCyberMoney", receivedCyberMoney);
            return "pay/exchange";
        }
        return "redirect:/user/exchange_apply"; // 성공 페이지로 리다이렉트합니다.
    }


    @PreAuthorize("isAuthenticated()")
    @GetMapping("/exchange_apply")
    public String exchange_applyPage(Model model, Principal principal) {
        String username = principal.getName();
        SiteUser user = userService.getUserbyName(username);
